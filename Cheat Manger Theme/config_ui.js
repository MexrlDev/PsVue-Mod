(function () {
  ...
  // ----- List positions -----
  var BASE_TEXT_X = 200;
  var TEXT_X = BASE_TEXT_X + LIST_OFFSET_X;

  var STATE_IMG_X = 1500 + LIST_OFFSET_X;
  var STATE_IMG_W = 36;
  var STATE_IMG_H = 50;

  var SWITCH_TEXT_X = (STATE_IMG_X - 40) + LIST_OFFSET_X;
  var SWITCH_TEXT_CENTER_OFFSET = 60;

  var LIST_START_Y = 550 + LIST_OFFSET_Y;
  var ITEM_HEIGHT = 70;
  ...
  var pressedKeys = {};
  var TEXT_HEIGHT = 38;
  var actualLineTargetW = 1600; // will be set in buildUI
  ...
  // Styles
  try { new Style({ name: 'title', color: 'black', size: 48, bold: true }); } catch (e) {}
  try { new Style({ name: 'listText', color: 'white', size: 36, bold: true }); } catch (e) {}  // white
  ...
  function buildUI() {
    ...
    var helpY = HELP_IMG_DEFAULT_Y; // 180
    ...
    for (var i = 0; i < configOptions.length; i++) {
      ...
      try {
        ...
        if (opt.type === 'toggle') {
          var imgX = SWITCH_TEXT_X + SWITCH_TEXT_CENTER_OFFSET - (STATE_IMG_W / 2);
          var img = new Image({
    ...
    try {
      ...
      // Back
      var backIconX = startX + selectSectionWidth + FOOTER_GAP_SELECT_BACK;
  ...
  function updateListPositions() {
    try {
      ...
      var baseYList = LIST_START_Y;

      for (var i = 0; i < optionTexts.length; i++) {
        var baseY = baseYList + i * ITEM_HEIGHT + (ITEM_HEIGHT - TEXT_HEIGHT) / 2;
        ...
        if (stateObj) {
          ...
          if (configOptions[i].type === 'toggle') {
            stateBaseY = baseYList + i * ITEM_HEIGHT + (ITEM_HEIGHT - STATE_IMG_H) / 2;
          } else {
            stateBaseY = baseYList + i * ITEM_HEIGHT + (ITEM_HEIGHT - TEXT_HEIGHT) / 2;
          }
          ...
          try {
            if (stateObj.img) {
              ...
              setVisible(stateObj.img, (stateY >= VISIBLE_TOP - STATE_IMG_H && stateY <= VISIBLE_BOTTOM));
            }
          } catch (e) {}
          ...
          try {
            if (stateObj.text) {
              ...
              setVisible(stateObj.text, (stateY >= VISIBLE_TOP - TEXT_HEIGHT && stateY <= VISIBLE_BOTTOM));
            }
          } catch (e) {}
        }
      }
      ...
      if (configOptions.length > 0 && selBarImg) {
        var selectedBaseY = baseYList + currentIndex * ITEM_HEIGHT;
  ...
  function ensureVisible() {
    try {
      var baseYList = LIST_START_Y;
      var itemTop = baseYList + currentIndex * ITEM_HEIGHT - scrollOffset;
      var itemBottom = itemTop + ITEM_HEIGHT;
      if (itemTop < VISIBLE_TOP) {
        scrollOffset = Math.max(0, baseYList + currentIndex * ITEM_HEIGHT - VISIBLE_TOP);
      } else if (itemBottom > VISIBLE_BOTTOM) {
        scrollOffset = baseYList + currentIndex * ITEM_HEIGHT + ITEM_HEIGHT - VISIBLE_BOTTOM;
      }
    } catch (e) {}
  }
  ...
  function cleanup() {
    ...
    try {
      ...
      lineImg = iconImg = titleLeft = selBarImg = footerSelectIcon = footerSelectText = footerBackIcon = footerBackText = helpBgImg = null;
    } catch (e) {}
  }
