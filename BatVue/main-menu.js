(function () {
  ...
  // store base size for subtle dynamic scaling
  background._baseWidth = background.width;
  background._baseHeight = background.height;
  ...
  // --- Background subtle expand/contract animation ---
  // small dynamic loop: expand slightly then return to normal repeatedly
  var _bgIdle = {
    phase: Math.random() * Math.PI * 2,
    speed: 0.12,   // cycles per second (0.12 Hz -> ~8.3s per cycle)
    amp: 0.02     // ±2% scale
  };
  var _bgIdleStart = Date.now();
  jsmaf.setInterval(function () {
    var bt = (Date.now() - _bgIdleStart) / 1000;
    var s = 1 + Math.sin(bt * _bgIdle.speed * 2 * Math.PI + _bgIdle.phase) * _bgIdle.amp;
    // apply uniform scale to avoid stretching
    background.scaleX = s;
    background.scaleY = s;
    // keep background visually centered while scaling
    var scaledW = background._baseWidth * s;
    var scaledH = background._baseHeight * s;
    background.x = Math.round((1920 - scaledW) / 2);
    background.y = Math.round((1080 - scaledH) / 2);
  }, 60);
