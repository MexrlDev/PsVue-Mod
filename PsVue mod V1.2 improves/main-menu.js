...
(function () {
  ...
  // places for buttons like.. reload-anim and reload app and exit.
  var trioY = startY + menuOptions.length * buttonSpacing + 40; // reduced from +100 to +40
  var trioGap = 30;

  // We'll place two visible trio buttons (reload-anim and reload-app) centered as a pair,
  // with a small gap between them so their combined center aligns with centerX.
  var smallGap = 10; // small gap between the two buttons

  // compute trioStartX so the two-button pair is centered at centerX
  ...
  // Reload-App (right of the pair)
  var reloadAppX = pairStartX + buttonWidth + smallGap;
  ...
  function entrance() {
    ...
    for (var idx = 0; idx < buttons.length; idx++) {
      (function (i) {
        ...
        // wrap timeout callback defensively
        (function (ii, cbDelay) {
          jsmaf.setTimeout(function () {
            ...
            try {
              ...
              animate(b, { alpha: 0, rotation: 360, y: (buttonOrigPos[ii] ? buttonOrigPos[ii].y + 40 : 0), scaleX: 0.6, scaleY: 0.6 }, { alpha: 1, rotation: 0, y: (buttonOrigPos[ii] ? buttonOrigPos[ii].y : 0), scaleX: 1.08, scaleY: 0.92 }, btnDuration, null, function () {
                // nested animations will also early-out if __appExiting
  ...
  // --- Graceful shutdown helper (robust/resilient)
  function _gracefulShutdownAndExit() {
    ...
    try {
      ...
      // also clear any explicit intervals/handles we kept
      try {
      ...
      // clear click/idles handles arrays if present
      try {
      ...
      // stop any active audio (bgm and click), defensively
      try {
      ...
      // defensive attempt to stop any global/persistent bgm (older scripts used __persistentBgm)
      try {
      ...
      try {
        if (typeof fn !== 'undefined' && typeof fn.getpid === 'function' && typeof fn.kill === 'function') {
          ...
          } catch (ke2) { log('low-level kill attempt failed: ' + (ke2 && ke2.message ? ke2.message : ke2)); }
        } else {
  ...
  // ensure highlight is set after a short delay so first button anim completes
  jsmaf.setTimeout(function () {
    // ensure currentButton is not the hidden exit index
