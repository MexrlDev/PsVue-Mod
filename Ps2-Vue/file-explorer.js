...
// Animated background (BgAnim folder) — 60 frames (frame_000 .. frame_059), avg duration 50ms, ~20 FPS
var bgBase = 'file:///../download0/BgAnim/frame_';
var bgExt = '.png';
var bgFramesTotal = 60; // total frames
var bgFrameIntervalMs = 50; // average frame duration in ms (given)
var bgFPS = Math.round(1000 / bgFrameIntervalMs); // approximate FPS (~20)
var bgFrameIndex = 0;
var bgUrls = [];
for (var bi = 0; bi < bgFramesTotal; bi++) {
  var idx = ('000' + bi).slice(-3);
  bgUrls.push(bgBase + idx + bgExt);
}
...
jsmaf.root.children.push(pr);
  ...
  var BG_MIN_LOADED_TO_START = 20;
  var BG_FALLBACK_MS = 3000;
  ...
  // Store base position for logo animation
  ...
  // Helpers (no tweens, immediate changes to avoid traces and speed up)
  ...
  // Orange dot loop (marker blink) - we'll integrate with idle loop, so this can be a simple static enable
  ...
  // Logo animated loop (replaces static startLogoLoop)
  function startLogoLoop() {
    ...
    logoAnimInterval = jsmaf.setInterval(function () {
      ...
      var bob = Math.sin(t * 1.6) * 6;         // vertical ±6px
      var sway = Math.sin(t * 0.9) * 4;        // horizontal ±4px
      var rot = Math.sin(t * 0.6) * 1.1;       // rotation ±1.1°
      var breath = 1.0 + Math.sin(t * 1.2) * 0.02; // scale ±2%
      try {
  ...
  // filesystem scanning and preview (kept efficient)
  ...
  // --- build UI grid (fast, immediate updates) with idle params ---
  function buildGrid() {
    if (!__explorerRunning) return;
    ...
    // Reset button arrays and idle params
    ...
    // create items quickly, set immediate visible state (no staggered entrance)
    for (var i = 0; i < filteredEntries.length; i++) {
      ...
      // ensure immediate visible state
    ...
    // Update highlight: ensure only current has selected visuals
    updateHighlight();

    // Start button idle loop
    startButtonIdleLoop();

    // immediate marker/logo state
    startOrangeDotLoop();
    startLogoLoop();
  ...
  // --- highlight (strict: only current shows selected visuals)
  var prevButton = -1;
  function updateHighlight() {
    ...
    // First: reset all buttons to neutral (ensures no trace left behind)
    for (var r = 0; r < buttons.length; r++) {
      try {
        var rb = buttons[r];
        if (!rb) continue;
        rb.url = 'file:///../download0/img/button_over_9.png';
        rb.alpha = 0.95;
        rb.borderColor = 'transparent';
        rb.borderWidth = 0;
        // scale and position will be handled by idle loop, so don't reset here
      } catch (e) {}
      try {
    ...
    // Then: apply selected visuals only to currentButton
    var i = currentButton;
    var b = buttons[i];
    if (b) {
      try {
  ...
  function navigateUp() {
    try {
      ...
      if (!pathStack || pathStack.length <= 1 || currentPath === startPath) {
        try {
          // Immediately stop explorer and switch to main menu
  ...
  (function startGamepadMouse() {
    ...
    var gpDeadzone = 0.15;
    var lastGpButtons = [];
    var gpPollStepMs = 33; // reduced frequency for lower CPU
    try {
  ...
  // --- FULL UNLOAD: completely stop script, clear intervals/timeouts, stop audio, remove visuals & handlers
  function fullUnload() {
    try {
      if (!__explorerRunning) return;
