(function () {
  ...
  // Base paths for Vue-Mod 1.3 assets (except button_over_9.png which uses /assets/img)
  ...
  // Use the global /assets/img for button_over_9.png as requested
  var normalButtonImg = 'file:///assets/img/button_over_9.png';
  var selectedButtonImg = 'file:///assets/img/button_over_9.png';
  jsmaf.root.children.length = 0;

  // Define white style (already present, but ensure it's applied to all text)
  ...
  // logo initial placement — start with alpha 0 so it fades in with the menu
  var logo = new Image({
  ...
  // Note: script names include '_Mod' to match the mod folder.
  var menuOptions = [
    { label: lang.jailbreak, script: 'loader.js', imgKey: 'jailbreak' },
    { label: lang.payloadMenu, script: 'payload_host_Mod.js', imgKey: 'payloadMenu' },
    { label: lang.config, script: 'config_ui_Mod.js', imgKey: 'config' },
    { label: lang.fileExplorer || 'File Explorer', script: 'file-explorer_Mod.js', imgKey: 'file_explorer' },
    { label: 'Back To Main-Menu', script: 'main-menu.js', imgKey: 'backToMain' },
  ...
  for (var i = 0; i < menuOptions.length; i++) {
    (function (i) {
      ...
      if (typeof useImageText !== 'undefined' && useImageText) {
        ...
        // Force white color explicitly (fix for text turning black)
        btnText.color = 'white';
        btnText.size = 24;
        btnText.style = 'white'; // also set named style for compatibility
        btnText.alpha = 0.0;
      }
      ...
      (function startEntrance(btn, text, marker, idx) {
        ...
        var interval = jsmaf.setInterval(function () {
          ...
          // Ensure text remains white (color property persists)
          text.color = 'white';
  ...
  function updateHighlight() {
    ...
    if (prevButton >= 0 && prevButton !== currentButton) {
      ...
      if (prevBtn && prevTxt && prevMark) {
        ...
        // Re-assert white color and style
        if (prevTxt) {
          prevTxt.color = 'white';
          prevTxt.size = 24;
          prevTxt.style = 'white';
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      // Ensure text is white at all times
      if (t) {
        t.color = 'white';
        t.size = 24;
        t.style = 'white';
  ...
  jsmaf.onKeyDown = function (keyCode) {
    if (keyCode === 6 || keyCode === 5) {
      ...
      // shortcut: Back To Main-Menu (load main-menu.js) — find the menu option and perform it
      for (var k = 0; k < menuOptions.length; k++) {
        if (menuOptions[k] && menuOptions[k].script === 'main-menu.js') {
          // animate press for that button if it exists and isn't entering; otherwise just perform
          (function (idx) {
            var btn = buttons[idx];
            var txt = buttonTexts[idx];
            if (!btn || !txt) {
              performButtonAction(idx);
              return;
            }
            if (btn._entering) {
              btn._entering = false;
              btn.x = buttonOrigPos[idx].x;
              btn.y = buttonOrigPos[idx].y;
              txt.x = textCenterPos[idx].cx;
              txt.y = textCenterPos[idx].cy;
              var mk2 = buttonMarkers[idx];
              if (mk2) { mk2.x = btn.x + buttonWidth - 50; mk2.y = btn.y + 35; mk2.alpha = 1.0; mk2.scaleX = 1.0; mk2.scaleY = 1.0; }
              performButtonAction(idx);
            } else {
              animatePress(btn, txt, buttonOrigPos[idx].x, buttonOrigPos[idx].y, textCenterPos[idx].cx, textCenterPos[idx].cy, function () {
                performButtonAction(idx);
              });
            }
          })(k);
