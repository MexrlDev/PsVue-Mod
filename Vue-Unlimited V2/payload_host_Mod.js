(function () {
  ...
  // placed on the left side of the screen and remains fixed
  ...
  function startMarkerBlink(marker) {
    ...
    // ensure marker is visible immediately when focused
    marker.visible = true;
  ...
  function stopMarkerBlink(marker) {
    ...
    // ensure marker is hidden when stopped
  ...
  function updateHighlight() {
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      if (i === currentButton) {
        ...
        b.borderWidth = 3;
        if (m) {
          // ensure previous markers are not left visible
          stopMarkerBlink(m);
          m.visible = true;
          startMarkerBlink(m);
        }
  ...
  function performButtonAction(index) {
    ...
    if (index === exitIndex) {
      ...
      try {
        include('../download0/PxMod/Vue-Mod 1.3/main-menu.js');
      } catch (e) {
        var err = e;
