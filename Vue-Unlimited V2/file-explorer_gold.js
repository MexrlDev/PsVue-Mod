...
(function () {
  ...
  // --- Base paths (use these for all mod assets) ---
  ...
  try {
    ...
    if (typeof global !== 'undefined' && global.__explorerBgm && global.__explorerBgm._valid) {
      ...
      try { bgm.open(sfxPath + 'explorer_bgm.wav'); } catch (e) {}  // fixed path
  ...
  var background = new Image({ url: imgPath + 'multiview_bg_VAF.png', x: 0, y: 0, width: 1920, height: 1080 });

  background.alpha = 0; background._baseX = background.x; jsmaf.root.children.push(background);

  var logo = new Image({ url: imgPath + 'logo.png', x: 1620, y: 0, width: 300, height: 169 });
  ...
  var searchBox = new Image({ url: imgPath + 'button_over_9.png', x: 130, y: 140, width: 420, height: 56 });
  ...
  // --- ORIGINAL breadcrumb (older) kept but hidden so it won't show
  ...
  // --- NEW: top-left path display so currentPath is more visible and stays visible
  // IMPORTANT: do NOT push this element into createdElements so hideCreated() won't hide it.
  ...
  // do NOT call pushCreated(pathTopLeft);
  ...
  modalCloseBtn = new Image({ url: imgPath + 'button_over_9.png', x: 1600, y: 100, width: 120, height: 46 });
  ...
  cursor = new Image({ url: imgPath + 'cursor.png', x: virtualMouse.x - cursorSize.w/2, y: virtualMouse.y - cursorSize.h/2, width: cursorSize.w, height: cursorSize.h });
  ...
  function buildGrid() {
    ...
    for (var i = 0; i < filteredEntries.length; i++) {
      ...
      try {
        ...
        btn = new Image({ url: imgPath + 'button_over_9.png', x: bx, y: by, width: buttonWidth, height: buttonHeight });
    ...
    if (currentPath !== startPath) {
      ...
      try {
        ...
        var upBtn = new Image({ url: imgPath + 'button_over_9.png', x: upBtnX, y: upBtnY, width: 180, height: 32 });
    ...
    // keep original breadcrumb hidden; update top-left path instead
    breadcrumbText.visible = false;
    breadcrumbText.alpha = 0;

    // update top-left path display whenever grid is built and ensure it's visible
  ...
  function closeModalAndResumeBgm() {
    ...
    // ensure top-left path remains visible after modal close
  ...
  function updateHighlight() {
    ...
    if (prevButton >= 0 && prevButton !== currentButton && buttons[prevButton]) {
      ...
      try { pb.url = imgPath + 'button_over_9.png'; pb.alpha = 0.8; } catch (e) {}
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      if (i === currentButton) {
        ...
        try { b.url = imgPath + 'button_over_9.png'; b.alpha = 1.0; b.borderColor = 'rgb(122, 91, 6)'; b.borderWidth = 3; } catch (e) {}
    ...
    // ensure top-left path remains visible and updated when highlight changes
    try { pathTopLeft.visible = true; pathTopLeft.text = currentPath; } catch (e) {}
      ...
      if (!pathStack || pathStack.length <= 1 || currentPath === startPath) {
        ...
        try {
          ...
          fadeOutBgm(300, function () {
            ...
            try { include('../download0/PxMod/Gold-Vue 1.2/main-menu.js'); } catch (e) { log('Could not include main-menu.js: ' + (e.message || e)); }
          ...
          try { include('../download0/PxMod/Gold-Vue 1.2/main-menu.js'); } catch (e) { log('navigateUp include error: ' + (e.message || e)); }
            ...
            try {
              ...
              if (currentPath === startPath) {
                ...
                fadeOutBgm(250, function () {
                  ...
                  try { include('../download0/PxMod/Gold-Vue 1.2/main-menu.js'); } catch (e) {}
  ...
  function enforceTextWhite() {
    ...
    // ensure top-left path uses small style too
  ...
  // Modified to preserve the new top-left path display (pathTopLeft) so it remains visible across unloads.
  ...
  function fullUnload() {
    ...
    try {
      ...
      // remove visuals from root but preserve pathTopLeft so it stays visible
      try {
        if (jsmaf && jsmaf.root && Array.isArray(jsmaf.root.children)) {
          ...
          // re-add preserved element if present
          if (preserved) {
      ...
      // drop local references to let GC collect, but keep pathTopLeft reference intact
      try {
        ...
        // DO NOT clear pathTopLeft here; keep it so the new path display remains persistent
