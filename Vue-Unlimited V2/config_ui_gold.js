...
(function () {
  ...
  for (var i = 0; i < configOptions.length; i++) {
    (function (i) {
      ...
      if (configOption.type === 'toggle') {
        valueEl = new Image({
          url: currentConfig[configOption.key] ? imgPath + 'check_small_on.png' : imgPath + 'check_small_off.png',
  ...
  function updateValueText(idx) {
    ...
    if (!opt || !valEl) return;
    if (opt.type === 'toggle') {
      valEl.url = currentConfig[opt.key] ? imgPath + 'check_small_on.png' : imgPath + 'check_small_off.png';
    } else {
      if (useImageText) {
