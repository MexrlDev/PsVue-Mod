...
(function () {
  ...
  function globalCleanup() {
    try {
      var bgmCandidates = [
        'bgmClip', '__explorerBgm', '__mainMenuBgm', '__menuBgm',
        'menuBgm', 'mainBgm', '_bgm', 'bgm'
      ];
      for (var i = 0; i < bgmCandidates.length; i++) {
        ...
        if (obj && typeof obj.stop === 'function') {
          ...
          try { if (typeof obj.pause === 'function') obj.pause(); } catch (e) {}
    ...
    } catch (e) { log('Cleanup error: ' + (e.message || e)); }
  }
  globalCleanup();

  // Syscall registration (BTW find more in my repo, on the folder of JSMAF.. youll find a lot of useful stuff for development :3)
  ...
  try { fn.register(0x80, 'rename', ['bigint', 'bigint'], 'bigint'); } catch (e) {}
  ...
  function isErrorResult(ret) {
    ...
    try {
      if (typeof ret.eq === 'function') {
        try { if (ret.eq(new BigInt(0xffffffff, 0xffffffff))) return true; } catch (e1) {}
        try { if (ret.eq(new BigInt(0xffffffffffffffff))) return true; } catch (e2) {}
      }
    } catch (e) {}
  ...
  function getFileIconUrl(item) {
    if (item.isDir) return folderIconUrl;
  ...
  function promptLater(title, initial, maxLen, cb) {
    jsmaf.setTimeout(function () {
      try {
        jsmaf.showOSK(title, initial, maxLen, cb);
      } catch (e) {
        showError('OSK failed: ' + (e.message || e));
      }
    }, 50);
  }
  ...
  function resolveMoveTarget(destInput, itemName) {
    var raw = (destInput === null || typeof destInput === 'undefined') ? '' : String(destInput).trim();
    if (!raw) return null;

    var wantsDirectory = raw.length > 0 && raw.charAt(raw.length - 1) === '/';
    var path = parseUserPath(raw);

    if (wantsDirectory) return joinPath(path, itemName);
    return path;
  }
  ...
  function selectedCount() {
    ...
    for (var k in selectedMap) {
      if (selectedMap.hasOwnProperty(k) && selectedMap[k]) c++;
    }
    return c;
  }
  ...
  function getSelectedItems() {
    ...
    for (var i = 0; i < allItems.length; i++) {
      if (selectedMap[allItems[i].path]) out.push(allItems[i]);
    }
    return out;
  }
  ...
  function toggleSelectAll() {
    ...
    for (var i = 0; i < allItems.length; i++) {
      if (!selectedMap[allItems[i].path]) {
        allSelected = false;
        break;
      }
    }
    ...
    if (allSelected) {
      ...
      for (var j = 0; j < allItems.length; j++) {
        selectedMap[allItems[j].path] = true;
      }
    }
    updateListUI();
  }
  ...
  var popupSelected = 0;
  var popupOptions = ['Rename', 'Delete', 'Create', 'Move', 'Edit', 'Run'];
      ...
      if (rlen > 0) {
        ...
        while (offset < rlen) {
          try {
            ...
            if (name && name !== '.' && name !== '..') {
              var itemPath = joinPath(path, name);
              var isDir = (d_type !== 8);
              results.push({ name: name, path: itemPath, isDir: isDir });
            }
  ...
  function readFileContent(path, callback) {
    try {
      ...
      if (!buf) {
        try { fn.close_sys(fd); } catch (e) {}
        return callback(new Error('Out of memory'));
      }
      ...
      var content = '';
      for (var i = 0; i < len; i++) {
        content += String.fromCharCode(mem.view(buf).getUint8(i));
      }
  ...
  function writeFileContent(path, content, callback) {
    try {
      ...
      if (!contentAddr) {
        try { fn.close_sys(fd); } catch (e) {}
        return callback(new Error('Memory allocation failed'));
      }
  ...
  function renameFileOrFolder(oldPath, newPath, callback) {
    try {
      ...
      var ret = null;
      if (typeof fn.rename_sys === 'function') ret = fn.rename_sys(oldAddr, newAddr);
      else if (typeof fn.rename === 'function') ret = fn.rename(oldAddr, newAddr);
      else return callback(new Error('rename syscall is not available'));
  ...
  function deleteItem(item, callback) {
    try {
      var addr = strToAddr(normalizePath(item.path));
        ...
        promptLater('Type empty to create blank file, or no to add content after', 'empty', 16, function (choice) {
          ...
          if (choice === 'no') {
            createFile(filePath, function (err2) {
              if (err2) {
                showError('Create file failed: ' + (err2.message || err2));
                return;
              }
  ...
  function runJsFile(item) {
    ...
    readFileContent(item.path, function (err, content) {
      if (err) {
        showError('Run failed: ' + (err.message || err));
        return;
      }
  ...
  function resetRowVisibility() {
    ...
    for (var j = 0; j < rowSelectedBackgrounds.length; j++) {
      if (rowSelectedBackgrounds[j]) rowSelectedBackgrounds[j].visible = false;
    }
    lastSelectedIndex = -1;
  }
  ...
  function updateListUI() {
    ...
    if (scrollOffset > allItems.length - visibleCount && allItems.length > visibleCount) {
      scrollOffset = allItems.length - visibleCount;
    }
    if (scrollOffset < 0) scrollOffset = 0;
  ...
  function refreshDirectoryData() {
    ...
    var timeout = jsmaf.setTimeout(function () {
      updateListUI();
    }, 16);
  ...
  function navigateUp() {
    ...
    if (persistentPathText) persistentPathText.text = currentPath;
  }
  ...
  function navigateDown() {
    if (viewerActive) {
      if (viewerContent) {
        viewerScroll += 28;
        viewerContent.y = 180 + viewerScroll;
      }
      return;
    }
  ...
  // Popup Menu
  function showPopup() {
    if (viewerActive || popupActive) return;
    popupActive = true;
    ...
    var optionHeight = 60;
    var startY = popupY + 80;
    popupSelected = 0;
    popupOptionBgs = [];
    popupOptionTexts = [];

    for (var i2 = 0; i2 < popupOptions.length; i2++) {
      var y = startY + i2 * optionHeight;
      var optBg = new Image({ url: selectImgUrl, x: popupX, y: y, width: popupWidth, height: optionHeight });
  ...
  function popupNavigate(delta) {
    ...
    for (var i = 0; i < popupOptionBgs.length; i++) {
      popupOptionBgs[i].visible = (i === popupSelected);
    }
  }
  ...
  function popupConfirm() {
    ...
    switch (option) {
      case 'Rename':
        if (multiMode) {
          showError("nuh uh, you're on multiple select.");
          return;
        }
        promptLater(
      ...
      case 'Delete':
        if (multiMode) {
          promptLater(
            'Delete selected items? type yes',
            '',
            8,
            function (answer) {
        ...
        if (multiMode) {
          promptLater(
            ...
            function (folderName) {
              folderName = sanitizeName(folderName);
              if (folderName) {
                createFolderAndMoveSelected(folderName, multiItems.slice(0));
              }
            }
          );
        } else {
          promptLater(
            'Create type: folder or file',
            '',
            16,
            function (type) {
              if (!type) return;
      ...
      case 'Move':
        if (multiMode) {
          ...
          promptLater(
            'Move selected items to path (use trailing /)',
            defaultDestMulti,
            512,
            function (dest) {
              if (dest && String(dest).trim() !== '') {
                var destPath = parseUserPath(dest);
                var destDir = destPath;

                if (String(dest).trim().charAt(String(dest).trim().length - 1) !== '/') {
                  destDir = parseUserPath(dest);
                }
          ...
          promptLater(
            'Move to path (use trailing / for folder)',
            defaultDest,
            512,
            function (dest2) {
      ...
      case 'Edit':
        if (multiMode) {
          showError('Hell No, you are in multiple select.');
          return;
        }
        if (item.isDir) {
          showError('Cannot edit a folder.');
          return;
        }
        readFileContent(item.path, function (err4, content) {
          if (err4) {
            showError('Failed to read file: ' + (err4.message || err4));
            return;
          }
          promptLater(
            'Edit file: ' + item.name,
            content,
            Math.max(4096, content.length + 1024),
            function (newContent) {
              if (newContent !== undefined && newContent !== null) {
                writeFileContent(item.path, newContent, function (writeErr) {
                  ...
                  else showError('File saved successfully.');
                });
              }
            }
          );
  ...
  function openTextViewer(item) {
    ...
    readFileContent(item.path, function (err, text) {
      if (err) {
        showError('Cannot preview: ' + (err.message || err));
        return;
      }
      ...
      jsmaf.setTimeout(function () {
        try {
          viewerContent.text = (text && text.length > 0) ? text : '(empty)';
        } catch (e) {
          viewerContent.text = '(error rendering text)';
        }
      }, 50);
  ...
  jsmaf.onKeyDown = function (keyCode) {
    ...
    if (keyCode === 4) navigateUpInList();
    else if (keyCode === 6) navigateDown();
    else if (keyCode === 14) confirmSelection();
  ...
  function fullUnload() {
    try {
      ...
      for (var i = 0; i < intervals.length; i++) {
        try { jsmaf.clearInterval(intervals[i]); } catch (e) {}
      }
      for (var j = 0; j < timeouts.length; j++) {
        try { jsmaf.clearTimeout(timeouts[j]); } catch (e) {}
      }
      try { jsmaf.root.children.length = 0; } catch (e) {}
      try { globalCleanup(); } catch (e) {}
      jsmaf.onKeyDown = function () {};
      jsmaf.onMouseMove = function () {};
      jsmaf.onMouseDown = function () {};
      jsmaf.onKeyUp = function () {};
  ...
  var openingTimeout = jsmaf.setTimeout(function () {
    openingImage.visible = false;
    persistentPathText.visible = true;
    initBgm();
    refreshDirectoryData();
  }, 3000);
  timeouts.push(openingTimeout);
})();
