...
(function () {
  ...
  background.alpha = 1; background._baseX = background.x; jsmaf.root.children.push(background);

  var logo = new Image({ url: 'file:///../download0/img/logo.png', x: 1620, y: 0, width: 300, height: 169 });
  logo.alpha = 1; logo.scaleX = 1; logo.scaleY = 1; jsmaf.root.children.push(logo);
  ...
  function startLogoLoop() {
    if (!__explorerRunning) return;
    try { if (logo) { logo.y = logo.y; logo.scaleX = 1; logo.scaleY = 1; } } catch (e) {}
    try { if (background) background.x = background._baseX; } catch (e) {}
  }
  ...
  // --- build UI grid (fast, immediate updates)
  function buildGrid() {
    if (!__explorerRunning) return;
    ...
    for (var i = 0; i < filteredEntries.length; i++) {
      ...
      // ensure immediate visible state (no entrance)
    ...
    // set elements static (no idle movement)
    startButtonIdleLoopStatic();
  ...
  // button idle - static (no motion) to avoid traces and improve CPU
  function startButtonIdleLoopStatic() {
    try {
      for (var i = 0; i < buttons.length; i++) {
        try {
          ...
          b.scaleX = 1;
          b.scaleY = 1;
          if (t) { t.x = Math.round(b.x + 80); t.y = Math.round(b.y + 28); }
          if (ic) { ic.x = Math.round(b.x + 8); ic.y = Math.round(b.y + 10); }
        } catch (e) {}
      }
    } catch (e) { log('startButtonIdleLoopStatic error: ' + (e.message || e)); }
  }
  ...
  function updateHighlight() {
    ...
    for (var r = 0; r < buttons.length; r++) {
      ...
      try {
        if (buttonTexts[r]) { buttonTexts[r].scaleX = 1; buttonTexts[r].scaleY = 1; }
      } catch (e) {}
      try {
        if (buttonIcons[r]) { buttonIcons[r].scaleX = 1; buttonIcons[r].scaleY = 1; }
      } catch (e) {}
      try {
    ...
    try {
      if (buttonTexts[i]) { buttonTexts[i].scaleX = 1.06; buttonTexts[i].scaleY = 1.06; }
    } catch (e) {}
    try {
      if (buttonIcons[i]) { buttonIcons[i].scaleX = 1.06; buttonIcons[i].scaleY = 1.06; }
    } catch (e) {}
    try {
