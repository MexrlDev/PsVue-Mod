...
// jars config if you wanna edit it..
  ...
  var label_offset_x = [-40, -40, -40, -40, -40, -40, -40];
  var label_offset_y = [-25, -25, -25, -25, -25, -25, -25];
  ...
  var background, introImg, logoImg, logoTextImg, creditsText;
  ...
  // Music – using jsmaf.AudioClip ofc.. the old script 
  var bgm = null;

  var mainLoopInterval = null, mousePollInterval = null;

  // ==================== STYLES ====================
  new Style({ name: 'credits', color: 'white', size: 24 });
  ...
  // ==================== CREDITS ====================
  creditsText = new jsmaf.Text();
  creditsText.text = 'Created by MexrlDev';
  creditsText.style = 'credits';
  creditsText.x = 20;
  creditsText.y = HEIGHT - 40;
  ...
  function updatePositions() {
    ...
    for (i = 0; i < labels.length; i++) {
      var jr = jarRects[i];
      labels[i].x = jr.x + jr.width / 2 + label_offset_x[i];
      labels[i].y = jr.y - 10 - 24 + label_offset_y[i];
    }
  }
  updatePositions();

  // ==================== MOUSE TRACKING.. idk why but doesn't work ====================
  ...
  function setState(newState) {
    ...
    if (newState === 'INTRO') {
      ...
      // Start white flash if coming from INTRO lol
      if (prevState === 'INTRO') {
  ...
  function handleJarAction(index) {
    switch (index) {
      ...
      case 5: // Settings config_ui.js in themes/apollo
        ...
        break;
      case 6: // About About.js in themes/apollo (fixed path)
